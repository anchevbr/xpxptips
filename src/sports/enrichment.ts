...
import { fetchOddsForFixture, getAverageOdds } from './providers/odds-api';
...
export async function enrichFixture(fixture: Fixture): Promise<MatchData> {
  ...
  try {
    ...
    if (eventOdds) {
      ...
      const over25 = getAverageOdds(eventOdds, 'totals', 'Over');
      const under25 = getAverageOdds(eventOdds, 'totals', 'Under');
      const bttsYes = isSoccer ? getAverageOdds(eventOdds, 'btts', 'Yes') : null;
      const bttsNo = isSoccer ? getAverageOdds(eventOdds, 'btts', 'No') : null;

      // Most commonly offered totals line across bookmakers (e.g. 2.5 for soccer, 215.5 for basketball)
      const totalsLine = (() => {
        const counts = new Map<number, number>();
        for (const bk of eventOdds.bookmakers) {
          const market = bk.markets.find((m) => m.key === 'totals');
          const pt = market?.outcomes.find((o) => o.name === 'Over')?.point ?? market?.outcomes[0]?.point;
          if (pt !== undefined) counts.set(pt, (counts.get(pt) ?? 0) + 1);
        }
